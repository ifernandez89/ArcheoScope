#!/usr/bin/env python3
"""
Medición instrumental - Doggerland, Mar del Norte
Candidato 5: Llanura sumergida paleolítica
"""
...
async def measure_doggerland():
    from satellite_connectors.real_data_integrator import RealDataIntegrator
    
    # Coordenadas: Doggerland - Mar del Norte central
    center_lat = 54.5
    center_lon = 2.5
    ...
    region_name = "Doggerland Mar del Norte - Llanura Sumergida"
    
    print("="*80)
    print("MEDICION INSTRUMENTAL - CANDIDATO 5")
    print("="*80)
    print(f"Region: {region_name}")
    print(f"Centro: {center_lat:.4f}N, {center_lon:.4f}E")
    print(f"Bbox: [{lat_min:.4f}, {lat_max:.4f}] x [{lon_min:.4f}, {lon_max:.4f}]")
    print(f"Objetivo: Paleocauces, superficies elevadas, nodos de rutas")
    print("="*80)
...
asyncio.run(measure_doggerland())
