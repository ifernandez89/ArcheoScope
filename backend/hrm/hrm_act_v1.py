...
class HierarchicalReasoningModel_ACTV1Block(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, cos_sin: CosSin = None, **kwargs) -> torch.Tensor:
...
class HierarchicalReasoningModel_ACTV1ReasoningModule(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, input_injection: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        for layer in self.layers:
            hidden_states = layer(hidden_states=hidden_states, cos_sin=kwargs.get('cos_sin'))
        return hidden_states
...
class HierarchicalReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: HierarchicalReasoningModel_ACTV1Config) -> None:
        ...
        self.H_init = nn.Parameter(trunc_normal_init_(torch.empty(self.config.hidden_size, dtype=self.forward_dtype), std=1), requires_grad=False)
        self.L_init = nn.Parameter(trunc_normal_init_(torch.empty(self.config.hidden_size, dtype=self.forward_dtype), std=1), requires_grad=False)
