#!/usr/bin/env python3
"""
Medición instrumental - Patagonia, Argentina/Chile
Candidato 4: Lagos proglaciares - terrazas
"""
...
async def measure_patagonia():
    from satellite_connectors.real_data_integrator import RealDataIntegrator
    
    # Coordenadas: Meseta del Lago Buenos Aires / General Carrera
    center_lat = -46.5
    center_lon = -71.0
    ...
    region_name = "Patagonia Lago Buenos Aires - Terrazas"
    
    print("="*80)
    print("MEDICION INSTRUMENTAL - CANDIDATO 4")
    print("="*80)
    print(f"Region: {region_name}")
    print(f"Centro: {center_lat:.4f}S, {center_lon:.4f}W")
    print(f"Bbox: [{lat_min:.4f}, {lat_max:.4f}] x [{lon_min:.4f}, {lon_max:.4f}]")
    print(f"Objetivo: Terrazas lacustres, abrigos, sitios de paso")
    print("="*80)
