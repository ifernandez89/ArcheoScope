#!/usr/bin/env python3
"""
Test de Calibración con Tell Mesopotámico como TECHO
====================================================

A. PISO (Control Negativo) - Pampa Argentina
B. ZONA HABITABLE (Benchmark Real) - Laguna Veracruz
C. TECHO (Antropogénico) - Tell Brak, Mesopotamia
"""
...
async def main():
    """Ejecutar los 3 casos de calibración con Tell Mesopotámico."""
    
    print("="*80)
    print("🎯 TEST DE CALIBRACIÓN - ArcheoScope (Tell Mesopotámico)")
    print("="*80)
    print()
    print("A. PISO (Control Negativo)")
    print("B. ZONA HABITABLE (Benchmark Real)")
    print("C. TECHO (Antropogénico) - Tell Brak")
    print()
    ...
    # C. TECHO - Tell Brak (Mesopotamia)
    print("\n" + "🔴"*40)
    print("C. TECHO (Antropogénico) - Tell Brak, Mesopotamia")
    print("🔴"*40)
    
    resultado_c = await test_caso(
        nombre="C. TECHO - Tell Brak",
        lat=36.365,
        lon=43.124,
        size_km=15.0,
        ...
        justificacion="Estratigrafía humana real + multitemporalidad + NDVI diferencial + SAR heterogéneo + thermal urbano enterrado"
    )
    ...
    # Guardar resultados
    output_file = f"calibracion_tell_mesopotamico_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
